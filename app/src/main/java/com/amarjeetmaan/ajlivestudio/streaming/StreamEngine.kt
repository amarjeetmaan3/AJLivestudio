...
/**
 * Camera -> optional GPU overlay processor -> StreamPack encoder -> RTMP.
 *
...
class StreamEngine(private val context: Context) {
    ...
    fun muteAudio(muted: Boolean) {
        try {
            ...
            // Optional across StreamPack/device combinations.
        }
    }
    ...
    suspend fun isTorchAvailableAsync(): Boolean {
        return awaitCameraSource()?.settings?.flash?.isAvailable == true
    }
