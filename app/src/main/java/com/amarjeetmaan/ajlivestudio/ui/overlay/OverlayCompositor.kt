 ...
 *
 * Camera:
 *   Camera -> SurfaceTexture/OES texture
 *
 * Processing:
 *   OES camera texture -> OpenGL -> optional ARGB overlay
 *
 * Output:
 *   OpenGL -> every StreamPack output Surface
 *
 * No MediaProjection / screen capture is used.
 */
class OverlayCompositor : ISurfaceProcessorInternal {
    ...
    private var pendingOverlayBitmap:
        Bitmap? = null
    ...
    private val vertexBuffer =
        floatBuffer(FULLSCREEN_VERTS)

    private val cameraTexCoordBuffer =
        floatBuffer(CAMERA_TEXCOORDS)

    private val overlayTexCoordBuffer =
        floatBuffer(OVERLAY_TEXCOORDS)
    ...
    /**
     * Replaces the currently uploaded overlay.
     *
     * The Bitmap is copied because the caller may recycle/change its bitmap.
     */
    fun setOverlayBitmap(bitmap: Bitmap?) {
        ...
        runOnGlThread {
            ...
            /*
             * If a frame is waiting, render it with the new overlay.
             */
            if (
                framePending &&
                outputs.isNotEmpty()
    ...
    override fun createInputSurface(
        ...
        runOnGlThreadBlocking {
            ...
            /*
             * A processor should have only one current input surface.
             */
            surfaceTexture
    ...
    override fun addOutputSurface(
        ...
        runOnGlThread {
            ...
            /*
             * Never add the same output twice.
             */
            val existing =
                outputs.firstOrNull {
                    it.output === surfaceOutput
                }
            ...
            /*
             * A frame may already have arrived before
             * StreamPack attached the output.
             */
            if (framePending) {
                renderPendingFrame()
            }
    ...
    override fun setTimebase(
         ...
         *
         * SurfaceTexture.timestamp is the timestamp belonging
         * to the camera frame that was actually consumed.
         */
    }
    ...
    /**
     * Consumes exactly one pending SurfaceTexture frame and
     * sends it to every current StreamPack output.
     */
    private fun renderPendingFrame() {
         ...
         * IMPORTANT:
         *
         * updateTexImage() is called only after an output exists.
         * This prevents us from consuming a camera frame and
        ...
        try {
            ...
            Log.e(
                TAG,
                "SurfaceTexture.updateTexImage failed",
                t
            )
        ...
        if (timestampNs <= 0L) {
            ...
            Log.w(
                TAG,
                "Invalid SurfaceTexture timestamp: $timestampNs"
            )
        ...
        /*
         * Snapshot prevents concurrent modification while
         * outputs are being rendered.
         */
        val snapshot =
            outputs.toList()
        ...
        for (entry in snapshot) {
            ...
            /*
             * StreamPack supplies the output transform.
             * It is combined with the SurfaceTexture transform.
             */
            val outputMatrix =
                FloatArray(16)
            ...
            /*
             * Preserve the original camera frame timestamp.
             */
        ...
        /*
         * Restore a valid EGL surface for the next callback.
         */
        checkEglCurrent()
    }
    ...
    private fun drawOverlay() {
        ...
        /*
         * Android Canvas Bitmaps are normally premultiplied.
         * ONE + ONE_MINUS_SRC_ALPHA is therefore appropriate.
         */
        GLES20.glBlendFunc(
            GLES20.GL_ONE,
